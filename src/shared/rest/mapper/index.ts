...
import {JsonConverstionError} from './DecoratorMetadata';
...
export namespace ObjectMapper {
    ...
    export const deserializeArray = <T>(type: { new(): T }, json: Object): T[] => {
        ...
        if (!(json instanceof Array)) {
          throw new JsonConverstionError('json parameter must be instanceof Array', '' + json);
        }
    ...
    /**
     * Deserializes an array of enum types with the passed on JSON data.
     */
    export const deserializePseudoEnums = <T>(type: { new(): T }, json: string[]): T[] => {
      if (!(json instanceof Array)) {
        throw new JsonConverstionError('json parameter must be instanceof Array', '' + json);
      }
      if (!type['byAlias']) {
        throw new JsonConverstionError('Pseudo Enum must implement static method byAlias(string): ' + type['name'], '' + json);
      }
      const enums: T[] = json.map(en => {
        const anEnum: T = type['byAlias'](en);
        if (!anEnum) {
          throw new JsonConverstionError('enum value "' + en + '" for class ' + type['name'] + ' wasn\'t recognized', '' + json);
        }
        return anEnum;
      });
      return enums;
    ...
    export const deserialize = <T>(type: { new(): T }, json: Object): T => {
        if (!(json instanceof Object)) {
          throw new JsonConverstionError('json parameter must be instanceof Object', '' + json);
        }
